...
contract RiobitDelegate is StoppableShareable, RiobitShared {
    ...
    // All minted RBT are initially sent to RBTowner, obeying both weekly and total supply caps
    ...
    function _stake(address _staker, uint256 _value, bytes32 _tag, uint256 _tournamentID, uint256 _roundID, uint256 _confidence) stopInEmergency internal returns (bool ok) {
        ...
        require(_value > 0 || stake.amount > 0); // Can't stake zero RBT
    ...
    // Transfer RBT from RBTowner account using multisig
    ...
    // Allows RBTowner to withdraw on behalf of a data scientist some RBT that they've deposited into a pre-assigned address
