...
import "contracts/StoppableShareable.symbol";
import "contracts/RiobitShared.sol";

contract RiobitBackend is StoppableShareable, RiobitShared {                                                            
    ...
    string public name = "Riobit";
    string public symbol = "RBT";
    uint256 public decimals = 6;

    event DelegateChanged(address oldAddress, address newAddress);

    function RiobitBackend(address[] _owners, uint256 _num_required, uint256 _initial_disbursement) StoppableShareable(_owners, _num_required) {
        totalSupply = 0;
        total_minted = 0;
    ...
    function claimTokens(address _token) onlyOwner {
        ...
        RiobitBackend token = RiobitBackend(_token);
    ...
    function RBTTransfer(address _to, uint256 _value) onlyPayloadSize(2) returns(bool ok) {
        return delegateContract.delegatecall(bytes4(sha3("RBTTransfer(address,uint256)")), _to, _value);
    }
    ...
    function transferFrom(address _from, address _to, uint256 _value) stopInEmergency onlyPayloadSize(3) returns (bool ok) {
        require(!isOwner(_from) && _from != RBT); // Transfering from RBTowner can only be done with the RBTTransfer function
    ...
    // ERC20: Anyone with RBT can transfer RBT
